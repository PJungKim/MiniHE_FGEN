...
- 전압 표시 통합
  - USB-PD전원 : `+12V_PD`
  - DC/DC Isolated 전원 : `+12V_ISO`
  - *5V는 사용하지 않는다!*
  - Starlit 전원 : `+3V3`
  - 플마 전원 5ㅂ Isolation 사용 시 : `+5V_ISO`, `GND_ISO`, `-5V_ISO`

- ㄱㄴㄷ(GND) 도메인
  - 메인 그라운드(자체 함수 발생기 출력단을 제외한 전ᄔᅧᆼᄔᅧᆨ) - `GND`
  - 자체 함수 발생기 출력단 - `GND_ISO`

- ㄱㄴㄷ(GND) 도메인 점검
  - GND와 연결되어야 하는 전압 : `+12V_PD`, `+3V3`
  - GND_ISO와 연결되어야 하는 전압 : `+12V_ISO`, `+5V_ISO`와 같이 뒤에 반드시 `_ISO` 플래그 붙이기
  - +12V_PD와 GND_ISO가 같은 블록에 연결되어 있다거나(아이솔레이터 제외) 하면 청문회때 탈탈 털릴 수 있음.
...
- 디버깅 방법
  - 이 세계관에서는 회로설계자는 코드를 직접 건드리는 일은 없다.
  - 모든 코드는 ㅍㅌㄷㅇ를 통해 쁘띠쁘띠로 넘어가고, 쁘띠쁘띠를 통해 제어할 수 있도록 설계된다(디버깅 모드).
  - 디버깅 모드가 아닌 경우 모든 쁘띠쁘띠로 넘어가는 코드는 결과를 띄우지 않는다.
  - 쁘띠쁘띠를 통한 제어는 충분한 범위로 가능하도록 설계한다.
